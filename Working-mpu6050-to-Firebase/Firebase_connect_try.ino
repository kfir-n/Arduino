...
void loop() {
  if (Firebase.ready() && signupOK && (millis() - sendDataPrevMillis > 300 || sendDataPrevMillis == 0)) {
    ...
    if (Firebase.RTDB.getInt(&fbdo, "/isStart")) {  // Check if data retrieval is successful
      if (fbdo.dataType() == "int") {               // Check if data type is integer
        intValue = fbdo.intData();                  // Get integer value from Firebase
        if (intValue == 1) {                        // Check if integer value is 1
          delay(2000);                              // Delay for 2 seconds for readability

          // Read data from MPU1
          mpu.getMotion6(&ax, &ay, &az, &gx, &gy, &gz);
          int x1 = map(ax, -17000, 17000, 0, 255);  // X axis data from MPU1
          Serial.print("Axis X 1 = ");
          Serial.println(x1);

          // Static value for Axis X 2
          int x2 = 180;
          ...
          if (minNumber > diffX) {
            ...
            if (Firebase.RTDB.setInt(&fbdo, "/angle", diffX)) {
              Serial.print("Stored new min difference of X values: ");
              Serial.println(diffX);
            }
          }
        }
